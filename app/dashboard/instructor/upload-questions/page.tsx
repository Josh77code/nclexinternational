      ...
      <main className="flex-1 pt-24 pb-12 bg-white">
        <div className="max-w-3xl mx-auto px-4">
          <Card className="border-2 border-[#749DC8] bg-white">
            <CardHeader>
              <CardTitle className="text-2xl text-[#0A61C9]">Upload Weekly Questions (CSV)</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-[#F1F7F9] border border-[#749DC8] rounded-lg p-4 space-y-2">
                <p className="font-semibold text-[#0A61C9]">How Course-Based Question Uploads Work:</p>
                <ul className="text-sm text-[#0A61C9] space-y-1 list-disc list-inside">
              ...
              <div className="space-y-2">
                <Label htmlFor="course-select" className="text-sm font-medium text-gray-700">
                  Course: <span className="text-[#0A61C9] font-semibold">*Required for course-specific questions</span>
                </Label>
                <Select value={selectedCourseId} onValueChange={setSelectedCourseId}>
                  <SelectTrigger id="course-select" className="w-full border-2 border-[#749DC8] text-[#0A61C9]">
                    <SelectValue placeholder="Select a course (required for 5 courses setup)" className="text-[#0A61C9]" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none" className="text-[#0A61C9] focus:text-[#0A61C9]">General Questions (No specific course)</SelectItem>
                    {courses.map((course) => (
                      <SelectItem key={course.id} value={course.id} className="text-[#0A61C9] focus:text-[#0A61C9]">
                        {course.title}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <div className="bg-[#F1F7F9] border border-[#749DC8] rounded-lg p-3">
                  <p className="text-xs text-[#0A61C9] font-semibold">
                    <strong>⚠️ IMPORTANT for 5 Courses Setup:</strong>
                  </p>
                  <ul className="text-xs text-[#0A61C9] mt-1 space-y-1 list-disc list-inside">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button onClick={() => window.open('/questions-template.csv', '_blank')} variant="outline" className="border-2 border-[#749DC8] text-[#0A61C9]">
                  Download Template
                </Button>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <Button disabled={!file || isUploading || !selectedGrade} onClick={onSubmit} className="bg-[#0A61C9] hover:bg-[#0A61C9]/90">
                  {isUploading ? 'Uploading...' : 'Upload CSV'}
                </Button>
              </div>
              {!selectedGrade && (
                <p className="text-sm text-[#0A61C9]">Please select a student grade level before uploading.</p>
              )}
              {result && (
                <div className={`mt-4 p-3 rounded border ${result.ok ? 'bg-[#0A61C9]/10 border-[#3895D3]/30' : 'bg-[#072F5F]/10 border-[#072F5F]/30'}`}>
                  <pre className={`whitespace-pre-wrap text-sm ${result.ok ? 'text-[#0A61C9]' : 'text-[#0A61C9]'}`}>{JSON.stringify(result.data, null, 2)}</pre>
                </div>
              )}
            </CardContent>
          </Card>
